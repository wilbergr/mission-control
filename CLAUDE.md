...
- **Dragging a group** (named headers only — Ungrouped always renders last, so it isn't draggable) is still just a reorder: `moveGroup` rebuilds the hid order as the groups' member blocks in their new sequence, then ungrouped, and sends it through the same `app:historyMove`. It works from `lastPrev` (the entries as last rendered), **not the DOM**, because a collapsed group has no rows in the DOM yet its members must move with it. A group dropped on a *row* targets that row's group; dropped on Ungrouped it goes to the end. `prevDragHid` and `prevDragGroup` distinguish the two kinds of drag, and every row/header handler checks which is in flight.
